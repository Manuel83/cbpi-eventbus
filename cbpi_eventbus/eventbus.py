            ...
            for idx, content_obj in enumerate(content_array):
                ...
                if inspect.iscoroutinefunction(content_obj.method):
                    if content_obj.supports_future is True:
                        ...
                        asyncio.create_task(content_obj.method(**kwargs, topic = topic, future=fut))

                    else:
                        t = asyncio.create_task(content_obj.method(**kwargs, topic=topic))
                        t.add_done_callback(self._done)
                        self._jobs.add(t)
                        #await content_obj.method(**kwargs, topic=topic)
                else:
                    # only asnyc
                    pass
